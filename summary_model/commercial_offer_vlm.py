...
Перед формированием ответа обязательно сделай отдельный проход по ВСЕМ страницам
вне ценовой таблицы: проверь текст сразу после итоговой суммы, перед подписью и в
нижней части последней ценовой страницы. Не оставляй верхнеуровневые поля null,
если соответствующая надпись явно видна хотя бы на одной странице.

Особенно проверь:
- `delivery_term_text`: копируй целиком строку вида
  «Срок оказания Услуг: с даты заключения контракта по 21.08.2026»;
- `vat_text`, `vat_rate`, `vat_amount`, `vat_included`: строка
  «в том числе НДС 5% в размере 5 062 476,48» означает, что НДС включён;
- если в ценовых строках одновременно есть числовая ставка и «Без НДС», сохрани
